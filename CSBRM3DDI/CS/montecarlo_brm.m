function collisionCost = montecarlo_brm(MCnum, AA, BB, GG, CC, DD, LL, xbar, ...
...
for mc = 1:MCnum
    ...
    xhatPrior0_MC = mvnrnd(xbar(:,1), PhatPrior0, 1)';
    ...
    for k = 1:(N + 1)
        ...
        % Feedback process
%         if k == 1
%            z_MC(:,k) = xhatPrior0_MC - xbar(:,1) + LL(:,:,k) * ytilde_MC(:,k);
%         else
%            z_MC(:,k) = AA(:, :, k - 1) * z_MC(:,k-1) ...
%                + LL(:,:,k) * ytilde_MC(:,k);
%         end
        z_MC(:,k) = xhat_MC(:,k) - xbar(:,k);
...
%     hold on;
%     plot3(x_MC(1,:), x_MC(2,:), x_MC(3,:), 'color', 0.6 * ones(3, 1));
    
    if MeanCollisionCheck(x_MC(1:3,:), world, 3)
