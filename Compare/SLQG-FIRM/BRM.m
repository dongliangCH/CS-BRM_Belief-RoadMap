...
%     % Exclude the node itself as its' neighbor
...
%                 [CovCost, CollisionCost, K] = PRMCovC( Covs{i,1}, Covs{i,2}, Covs{near_idx(j), 1}, Covs{near_idx(j), 2}, N, Signal, param, Xbar, V, Cspace );   
%                     plot(param.scale * meanTraj(2,:), param.scale * meanTraj(1,:), 'color', 'g', 'LineWidth', 1);
%                     n = n + 1;
...
for i = 1:size(Path, 2)-1
    ...
    [CovCost, CollisionCost, K] = PRMCovC( Covs{Path(i),1}, Covs{Path(i),2}, Covs{Path(i+1), 1}, Covs{Path(i+1), 2}, N, Signal, param, Xbar, V, Cspace );   
end
