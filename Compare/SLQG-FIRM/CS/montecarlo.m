...
function collisionCost = montecarlo(MCnum, AA, BB, GG, CC, DD, LL, xbar, ...
    N, PhatPrior0, PtildePrior0, V, K, scale)
...
for mc = 1:MCnum
    ...
    xhatPrior0_MC = mvnrnd(xbar(:,1) - xbar(:,1), PhatPrior0, 1)';
    ...
    hold on;
    plot( (x_MC(2,:) + xbar(2,:)).*scale, (x_MC(1,:) + xbar(1,:)).*scale, 'color', 0.6 * ones(3, 1));
    
    if ~MeanCollisionCheck(scale * [x_MC(1,:) + xbar(1,:); x_MC(2,:) + xbar(2,:)], Cspace)
